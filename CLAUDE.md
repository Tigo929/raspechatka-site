...
**PRINTLAB** — премиальный коммерческий сайт студии печати на футболках.
...
`src/features/configurator`. Пользователь выбирает цвет футболки, загружает
изображение, двигает/масштабирует его, видит превью и оформляет заказ **только
по явной кнопке**. Загруженное изображение обрабатывается локально (FileReader,
object URL) — на сервер ничего не уходит. Это основа: расширяется до бэкенда/корзины.

## Конверсия (правила)

Каждый блок отвечает на вопрос: «приближает ли это к заказу?». Sticky CTA на
мобильных, быстрый контакт (Telegram/WhatsApp), отзывы, гарантии, отработка
