...
- **Админ-панель** (`/admin`) — заявки с повторной отправкой, каталог, контент,
  отзывы, FAQ, контакты, аналитика и загрузка изображений.
...
5. **Приём заявок** — обращение сначала сохраняется в `data/orders.json`, затем
   доставляется в Telegram. Недоставленные заявки видны в `/admin` и отправляются повторно.
...
Для продакшена задайте `PRINTLAB_DATA_DIR` на подключённый постоянный том: там
хранятся журнал заявок, приложенные макеты и внутренняя аналитика.
