...
server {
    ...
    # Увеличенный timeout на время, пока HTTP-ответ ожидает Telegram API.
    # TODO: вернуть 60s после внедрения persisted outbox — тогда HTTP-ответ
    # не будет ждать Telegram и этот таймаут станет избыточным.
...
## Шаг 13. Резервное копирование runtime-данных
